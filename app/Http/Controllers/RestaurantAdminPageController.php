...
class RestaurantAdminPageController extends Controller
{
    ...
    /**
     * added by code by monika for walkin order
     */
    public function addwalkinorder()
    {
        $transation = new TranslationService();
        $sanboxNumber = Setting::all()->where('key', 'PhoneCode')->first()->value;
        $category = Category::all()->where('store_id', auth()->id());
    ...
    {
        $price = '0';
        $products = Product::with('addonItems')->where('id', '=', $request->product_id)->first();
        if ($products) {
            $addonData = Addon::where(array('store_id' => auth()->id(), "addon_category_id" => $products['category_id']))->get();
            $products['addonData'] = $addonData;
        }
        if ($products) {
